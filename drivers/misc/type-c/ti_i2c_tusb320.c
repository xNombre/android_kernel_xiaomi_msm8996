...
static void ti_tusb320l_reset(struct ti_tusb320_data *ti_data)
{
	int ret;
	char buf;

	/* 1. write 0 to 0x45 reg, make sure the CC in normal status */
	buf = 0;
	ret = ti_tusb320_i2c_write(ti_data, TI_DISABLE_RD_RP_REG, &buf, 1);
	if (ret < 0)
		dev_info(&ti_data->i2c_client->dev, "set TI_DISABLE_RD_RP_REG fail\n");
	/* 2. write 1 to bit[3] of 0xA reg to reset the device */
	buf = 0x8;
	ret = ti_tusb320_i2c_write(ti_data, TI_ROLE_MODE_REG, &buf, 1);
	if (ret < 0)
		dev_info(&ti_data->i2c_client->dev, "reset chipset fail\n");
	msleep(100);
}

static int ti_tusb320l_set_trysnk_drp_mode(struct ti_tusb320_data *ti_data)
{
	int ret;
	char buf;

	/* 0. write 0 to 0x45 reg, make sure the CC in normal status */
	buf = 0;
	ret = ti_tusb320_i2c_write(ti_data, TI_DISABLE_RD_RP_REG, &buf, 1);
	if (ret < 0)
		dev_info(&ti_data->i2c_client->dev, "set TI_DISABLE_RD_RP_REG fail\n");
	...
	buf = buf | ((TRYSNK_DRP_MODE << TI_DRP_ROLE_OFFSET) & TI_DRP_ROLE_MODE_MASK)
			| ((DRP_MODE << TI_ROLE_OFFSET) & TI_ROLE_MODE_MASK);
...
{
	if (ti_data->i2c_client->addr == TI_TUSB320L_ADDR)
		ti_tusb320l_set_trysnk_drp_mode(ti_data);
		...
		const struct i2c_device_id *dev_id)
{
	...
	/* reset the chipset */
	if (client->addr == TI_TUSB320L_ADDR_1) {
		ti_tusb320l_reset(tusb320_data);
		dev_err(&client->dev, "The address is invalid\n");
		retval = -ENXIO;
		goto err_absent;
	}
	if (client->addr == TI_TUSB320L_ADDR) {

		retval = ti_tusb320l_set_trysnk_drp_mode(tusb320_data);
		if (retval < 0)
			dev_err(&client->dev,
